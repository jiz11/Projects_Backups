...
# lon和lat分别是要插值的点的x,y
# lst是已有数据的数组，结构为：[[x1，y1，a1，b1，c1]，[x2，y2，a2，b2，c2]，...]
# 返回值是插值点的三种温度
def interpolation(lon, lat, lst):
    ...
    avg_temp_interp = sum0_avg / sum1_avg if sum0_avg != 0 else -9999999
    high_temp_interp = sum0_high / sum1_high if sum0_high != 0 else -9999999
    low_temp_interp = sum0_low / sum1_low if sum0_low != 0 else -9999999
    return avg_temp_interp, high_temp_interp, low_temp_interp

all_district = []

def read_data():
    city = pd.read_excel('../map (administrative division)/city_geo.xlsx')

    for idx, row in city.iterrows():
        all_district.append(
            {'name': row['市'], 'citycode': row['市代码'], 'lon': row['LON'], 'lat': row['LAT']})


if __name__ == '__main__':
    read_data()
    dirs = os.listdir('../weather/raw data/SURF_CLI_CHN_MUL_DAY_V3.0/processed/splitday')
    exist_dirs = set(os.listdir('../weather/raw data/SURF_CLI_CHN_MUL_DAY_V3.0/processed/city_int'))
    for file_path in dirs:
        ...
        for district in all_district:
            ...
            results.append([str(district['name']), str(district['citycode']), avg_temp_interp, high_temp_interp, low_temp_interp])

        out = pd.DataFrame(results)
        out.to_csv('../weather/raw data/SURF_CLI_CHN_MUL_DAY_V3.0/processed/city_int/' + file_path, header=['cityname', 'citycode', 'avg_temp', 'high_temp', 'low_temp'])
